  ...
  public:
    ...
    string typeCheckStr(SymbolTable* parent) {
      if(expType == "relop") {
        ...
        string expType = children[1]->typeCheckStr(parent);
        if(expType == INVALIDSYM) {
          cerr << "Type Error: Invalid expression type at " << lnum << endl;
          return INVALIDSYM;
        } else if (expType != "int") {
          cerr << "Type Error: " << children[0]->getType() 
               << " only takes an integer argument at " << lnum << endl;
          return INVALIDSYM;
        }
        return expType;
    ...
    string typeCheckStr(SymbolTable* parent) {
      ...
      if(type == "this") {
        ...
        if(index > name.length()){
          ...
          cerr << "Type Error: Invalid dimension on array access at " 
               << lnum << endl;
          return INVALIDSYM;
        }
        name = name.substr(0, index);
